...
begin
  ...
  try
    ...
    begin
      MessageDlg('Campo usuário é obrigatório!', mtConfirmation, [mbOK], 0, mbOK);
      Abort;
    end;

    if Length(mUser.User) < 4 then
    begin
      MessageDlg('O usuário deve ter no minimo 4 caractéres!', mtConfirmation, [mbOK], 0, mbOK);
      Abort;
    end;

    if Trim(mUser.Password) = '' then
    begin
      MessageDlg('Campo senha é obrigatório!', mtConfirmation, [mbOK], 0, mbOK);
      Abort;
    end;

    if Length(mUser.Password) < 4 then
    begin
      MessageDlg('O senha deve ter no minimo 4 caractéres!', mtConfirmation, [mbOK], 0, mbOK);
      Abort;
    end;
